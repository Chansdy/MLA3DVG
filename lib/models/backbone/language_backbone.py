...
class LanguageBackboneModel(nn.Module):
    ...
    def forward(self, text, device, n,return_dict=None,):
        ...
        if n==1:
            ...
            hidden_states, attention_mask = self.backbone(**tokenized,n=torch.tensor(1, dtype=torch.int64))
        else:
            tokenized = self.tokenizer.batch_encode_plus(
                text, max_length=50, padding="max_length", truncation=True, return_tensors="pt"
            ).to(device)
            hidden_states, attention_mask = self.backbone(**tokenized,n=torch.tensor(0, dtype=torch.int64))
