...
from .configuration_mla3dvg import Mono3DVGConfig
...
@dataclass
class Mono3DVGDecoderOutput(ModelOutput):
    """
    Base class for outputs of the Mono3DVGDecoder. This class adds three attributes to
...
@dataclass
class Mono3DVGModelOutput(ModelOutput):
    """
    Base class for outputs of the Mono 3DVG encoder-decoder model.
...
@dataclass
class Mono3DVGForSingleObjectDetectionOutput(ModelOutput):
    """
    Output type of [`Mono3DVGForSingleObjectDetection`].
...
class AttrGuidedAdapter(nn.Module):
    def __init__(
        self, 
        ...
        self.spa2text_attn= nn.MultiheadAttention(d_model, n_heads, dropout=dropout, batch_first=True)
    ...
    def positional_encoding(self, seq_len, d_model=256) -> torch.Tensor:
    ...
    def forward(
        ...
        attr_pose_embed=self.positional_encoding(50,256).to(attr_v_embeds.device)
        k = self.with_pos_embed(attribute, attr_pose_embed)
        imgfeat_adapt, _ = self.img2text_attn(
            query=attr_v_embeds,
            key=k,
            value=k,
            key_padding_mask=attr_mask
        )
        spa_pose_embed=self.positional_encoding(50,256).to(space_embeds.device)
        k = self.with_pos_embed(spatial, spa_pose_embed)
        spafeat_adapt, _ = self.spa2text_attn(
            query=space_embeds,
            key=k,
            value=k,
            key_padding_mask=spa_mask
        )
        
        return imgfeat_adapt,spafeat_adapt
...
class Mono3DVGDecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        #attr
        self.cross_attn_attr = nn.MultiheadAttention(self.embed_dim, config.decoder_attention_heads, dropout=config.dropout, batch_first=True)
        self.cross_attn_attr_layer_norm = nn.LayerNorm(self.embed_dim)
    ...
    def forward(
        ...
        #For attr
        attr_adapt_k:Optional[Tensor] = None,
        attr_embeds:Optional[Tensor] = None,
    ):
        """
        Args:
        ...
        residual = hidden_states
         ...
         # Attr Cross Attention
        hidden_states, cross_attn_attr_weights = self.cross_attn_attr(
            query=hidden_states,
            key=attr_adapt_k,
            value=attr_embeds,
        )
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = self.cross_attn_attr_layer_norm(hidden_states)
...
class Mono3DVGPreTrainedModel(PreTrainedModel):
    config_class = Mono3DVGConfig
...
class Mono3DVGDecoder(Mono3DVGPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`Mono3DVGDecoderLayer`].
    ...
    def __init__(self, config: Mono3DVGConfig):
        super().__init__(config)
        
        self.dropout = config.dropout
        self.layers = nn.ModuleList([Mono3DVGDecoderLayer(config) for _ in range(config.decoder_layers)])
...
class Mono3DVGModel(Mono3DVGvPreTrainedModel):
    def __init__(self, config: Mono3DVGConfig):
        super().__init__(config)
        self.d_model = config.d_model
        ...
        # for param in self.depth_encode.parameters():
        ...
        #             nn.Conv2d(256, 256, kernel_size=3, stride=8, padding=1),
        #             nn.GroupNorm(32, 256)
        #         )
        ...
        if self.use_text_guided_adapter:
            ...
            self.attr_guided_adapter = AttrGuidedAdapter(config)
            self.downsample = nn.Sequential(
                    nn.Conv2d(256, 256, kernel_size=3, stride=8, padding=1),
                    nn.GroupNorm(32, 256)
                )
        ...
        self.decoder = Mono3DVGDecoder(config)
    ...
    def forward(
        ...
        space_embed:Optional[List[dict]] = None,
        attr_embed:Optional[List[dict]] = None,
        space_map:Optional[List[dict]] = None,
        attribute:Optional[List[Tuple[str]]] = None,
        spatial:Optional[List[Tuple[str]]] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.FloatTensor], Mono3DVGForSingleObjectDetectionOutput]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        text_features, text_attention_mask = self.language_backbone(text, device,n)
        ...
        #attribute&spatial feature
        n= torch.tensor(0, dtype=torch.int64)
        attr_features, attr_attention_mask = self.language_backbone(attribute, device,n)
        spa_features, spa_attention_mask = self.language_backbone(spatial, device,n)
        attr_embeds=self.language_proj[5](attr_features)
        spa_embeds=self.language_proj[5](spa_features)
        ...
        cls_embed=self.attr_proj(attr_embed[1].clone().unsqueeze(2).unsqueeze(3))
        attr_v_embeds=self.attr_proj(attr_embed[0].clone())
        space_embeds=F.interpolate(attr_v_embeds, size=(24, 80), mode='bilinear', align_corners=False)
        space_maps=space_map.clone().squeeze(1)
        ...
        attr_v_embeds=attr_v_embeds.flatten(2).permute(0, 2, 1)
        cls_embed=cls_embed.flatten(2).permute(0, 2, 1)
        attr_v_embeds=torch.cat((cls_embed,attr_v_embeds),dim=1)
        
        # Sixth, prepare attr-guided adapter 
        if self.use_attr_guided_adapter:

            attr_adapt_k,depth_adapt_k= self.attr_guided_adapter(
                space_embeds=depth_embeds, 
                attr_v_embeds=attr_v_embeds,
                spatial=spa_embeds,
                spa_mask=spa_attention_mask,
                attribute=attr_embeds,
                attr_mask=attr_attention_mask,
            )
        ...
        decoder_outputs = self.decoder(
            ...
            attr_embeds=attr_v_embeds,
            # attr_adapt_k=attr_adapt_k,
            attr_adapt_k=attr_v_embeds,
            output_attentions=output_attentions,
...
class Mono3DVGForSingleObjectDetection(Mono3DVGPreTrainedModel):
    
    def __init__(self, config: Mono3DVGv2Config):
        super().__init__(config)
        
        # Mono3DVG  model
        self.model = Mono3DVGModel(config)
    ...
    def forward(
        ...
        dino_input:Optional[List[dict]] = None,
        attribute:Optional[List[Tuple[str]]] = None,
        spatial:Optional[List[Tuple[str]]] = None,
        space_embed:Optional[List[dict]] = None,
        attr_embed:Optional[List[dict]] = None,
        ...
        if labels is not None:
            ...
            matcher = Mono3DVGHungarianMatcher(
        ...
        dict_outputs = Mono3DVGForSingleObjectDetectionOutput(
            loss=loss,
            loss_dict=loss_dict,
            logits=logits,
    ...
    def _load_mono3dvg_pretrain_model(
        ...
        config: Optional[Union[Mono3DVGConfig, dict]] = None,
        ...
        if config is None:
            ...
            assert isinstance(config, Mono3DVGConfig), f"config: {config} has to be of type Mono3DVGConfig"
...
class Mono3DVGHungarianMatcher(nn.Module):
