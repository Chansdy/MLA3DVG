    ...
    def __init__(self, root_dir, split: str):
        ...
        for i, ann in pbar:
            ...
            self.anns[i] = {
                ...
                'attribute': ann['object_attribute'],
                'spatial': ann['object_spatial'],
            }
...
class Mono3DReferDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        #dinov2-image:
        transform_p = make_depth_transform()
        scale_factor = 1
        rescaled_image = img.resize((scale_factor * img.width, scale_factor * img.height))
        transformed_image = transform_p(rescaled_image)
        ...
        #  ============================   get attribute&spatial  ==============================
        import random
        attribute_list=ann['attribute']
        spatial_list=ann['spatial']
        if len(attribute_list)>1:
            attribute=attribute_list[0]
            for i in range(1,len(attribute_list)):
                attribute=attribute+','+attribute_list[i]
        else:
            attribute=attribute_list[0]
        if len(spatial_list)>1:
            spatial=spatial_list[0]
            for i in range(1,len(spatial_list)):
                spatial=spatial+','+spatial_list[i]
        else:
            spatial=spatial_list[0]
        ...
        return {
            ...
            'info': info,
            'dinov2-input':transformed_image,
            'attribute':attribute,
            'spatial':spatial,
        }
